...
public:
  Motor() : Node("motor_simulator"), tau(0), w(0), th(0) {
    sub = create_subscription<std_msgs::msg::Float64>("torque_cmd", 10,
      [this](std_msgs::msg::Float64::SharedPtr m){ tau = m->data; });
    pub 
= create_publisher<motor_sim_msgs::msg::MotorState>("motor_state", 10);
    timer 
= create_wall_timer(10ms, [this]{ update(); });
  }
